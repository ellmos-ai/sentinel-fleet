...
pillar: control
description: >
  Two-tier model selector between Gemini 3.5 Flash and Gemini 3.7 Flash, keyed on a caller-supplied complexity label. Not wired into the live request path; the console's model dropdown decides the model in practice. No cost, token, or latency measurement feeds the selection, and no local/edge model is deployed.
...
## Purpose
Selects between two Gemini model tiers given a `task_complexity` label ("high" routes to the
newer generation, anything else to the standard one). There is no Pro tier to route to: the
Gemini API's models.list has never served a `gemini-3.5-pro`, so escalation means a newer
generation rather than a larger model.
