...
class GatewayExecutionResult:
    def __init__(self, success: bool, output: Any = None, error: Optional[str] = None, requires_approval: bool = False, approval_payload: Optional[Dict[str, Any]] = None):
...
class SovereignGateway:
    ...
    async def execute_tool_call(
        ...
        """Zero-Trust Interceptor for all agent actions."""
        ...
        try:
            ...
            # 3. Model Armor Inspection (Anti-Injection & PII Redaction)
            sanitized_args = self.model_armor.sanitize_arguments(tool_args)
